...
function populateQualitySelect(formatsDictionary) {
    ...
    // Якщо елемент не знайдено, перериваємо виконання
    if (!selectElement) {
        console.error("Елемент 'quality_select' не знайдено.");
        return;
    }

    // 1. Додаємо опцію за замовчуванням (підказку)
    // Використовуємо createElement для чистоти
    defaultOption = document.createElement('option');
    defaultOption.value = '';
    defaultOption.textContent = '-- Виберіть якість --';
    defaultOption.disabled = true;
    defaultOption.selected = true;
    selectElement.appendChild(defaultOption);

    // 2. Ітерація по словнику { якість: format_id }
    for (qualityKey in formatsDictionary) {
        // Перевіряємо, чи це власний ключ об'єкта
        if (formatsDictionary.hasOwnProperty(qualityKey)) {
            
            formatId = formatsDictionary[qualityKey];
            option = document.createElement('option');
            
            option.value = formatId; 

            let displayQuality = qualityKey;

            if (qualityKey !== 'audio-only') {
                displayQuality = `${qualityKey} (Відео + Аудіо)`;
                option.textContent = displayQuality;
